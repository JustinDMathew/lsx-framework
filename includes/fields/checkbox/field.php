<div class="row form-group">
	...
	<div class="col-sm-9">
		<?php foreach( $field['options'] as $value=>$label ){ ?>
			<div><input data-live-sync="true" name="{{:name}}[value][<?php echo $value; ?>]" type="checkbox" id="{{_id}}" value="<?php echo esc_attr( $value ); ?>" {{#is value value="<?php echo $value; ?>"}} selected="selected"{{/is}}> <?php echo esc_html( $label ); ?></div>
		<?php } ?>
