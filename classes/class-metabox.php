...
class Lsx_Metabox {
	...
	public function get_field_types( $field_types ) {
		...
		$internal_types = array(
			'text'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/text/field.php'
			),				
			'textarea'		=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/textarea/field.php',
			),
			'select'		=>	array(
			...
			'hours'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/hours/field.php',
			),
			'image'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/image/field.php',
				'handler'	=>	array( $this, 'handle_image' ),
				'scripts'	=> array(
					LSX_URL . 'includes/fields/image/image-picker.js',
				),
			),
			'color'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/color/field.php',
				'styles'	=> array(
					'wp-color-picker'
				),
				'scripts'	=> array(
			...
			'date'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/date/field.php',
				'styles'	=> array(
					LSX_URL . 'includes/fields/date/datepicker.css'
				),
				'scripts'	=> array(
					LSX_URL . 'includes/fields/date/bootstrap-datepicker.js'
				),
			),
			'html'			=>	array(
				'file'		=>	LSX_PATH . 'includes/fields/html/field.php',
			),
		);
	...
	public function load_plugin_textdomain() {
		...
		load_plugin_textdomain( $this->plugin_slug, FALSE, basename( LSX_PATH ) . '/languages');
	...
	public function add_metaboxes() {
		...
		wp_enqueue_script( 'lsx-metabox', LSX_URL . 'assets/js/metabox.js', array( 'jquery' ), LSX_VER, true );
		wp_enqueue_style( 'lsx-metabox', LSX_URL . 'assets/css/metabox.css');
			...
			if( !empty( $field_type['styles'])){
				foreach($field_type['styles'] as $style){
					if( false !== strpos($style, '//')){
						wp_enqueue_style( $field_type_slug . '-' . sanitize_key( basename( $style ) ), $style, array(), LSX_VER );
					}else{
			...
			if( !empty( $field_type['scripts'])){
				...
				foreach($field_type['scripts'] as $script){
					if( false !== strpos($script, '//')){
						wp_enqueue_script( $field_type_slug . '-' . sanitize_key( basename( $script ) ), $script, $depts, LSX_VER );
					}else{
	...
	public function render_metabox( $post, $metabox ) {
		...
		<?php include LSX_PATH . 'includes/templates/main-metabox.php'; ?>
		</script>
		<script>
	...
	public function enqueue_admin_stylescripts() {
		...
		if( false !== strpos( $screen->base, 'post' ) ){
			...
			wp_enqueue_style( 'lsx-metabox-forms', LSX_URL . '/assets/css/form.css' );
			wp_enqueue_script( 'lsx-wp-baldrick', LSX_URL . '/assets/js/wp-baldrick-full.js', array( 'jquery' ) , false, true );
